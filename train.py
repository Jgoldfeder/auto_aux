...
group.add_argument('--smooth', action='store_true', default=False)
group.add_argument('--dual', action='store_true', default=False)
group.add_argument('--name', default='', type=str, 
                    help='experiment name')
parser.add_argument('--weights', type=float, nargs='+', default=[.5,.5],
                            help='task weights.')
group.add_argument("--level", type=int, default=100, help="Data level.")
group.add_argument('--n-ways', type=int, default=1, metavar='S')
...
def main():
    ...
    model = create_model(
        ...
        checkpoint_path=args.initial_checkpoint,img_size=args.img_size)
    print("dual_mode:",args.dual)

    dual_mode = args.dual
    if dual_mode:
        if "vit" in args.model or "beit" in args.model or "mixer" in args.model:
            model = dual.DualModelVIT(model,args.num_classes,args.n_ways)
        else:
            model = dual.DualModel(model,args.num_classes,args.n_ways)
        model.dual_mode = True
    else:
        model.dual_mode = False
    ...
    if dual_mode:
        metabalancer  = dual.MetaBalance(model.model.parameters())
    ...
    model.dual_mode = dual_mode
    if dual_mode:
        model.metabalancer = metabalancer
    ...
    if dual_mode:
        print("Task weights:",args.weights)
        if args.n_ways !=1:
            train_loss_fn = dual.NWayLoss(train_loss_fn,args.weights,args.num_classes,args.n_ways)  
        elif args.smooth:
            train_loss_fn = dual.DualLossSmooth(train_loss_fn,args.weights,args.num_classes)  
        else:
            train_loss_fn = dual.DualLoss(train_loss_fn,args.weights,args.num_classes)  
    
    train_loss_fn = train_loss_fn.cuda()
    validate_loss_fn = train_loss_fn#nn.CrossEntropyLoss().cuda()
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            # if after 10 epochs no improvment, stop
            ...
            if epoch==num_epochs-1:  
                ...
                if args.dual:
                    labels =train_loss_fn.dense_labels[:args.num_classes,:]
                    if args.n_ways !=1:
                        labels = F.one_hot(labels, args.n_ways)
                        labels= labels.reshape(args.num_classes,train_loss_fn.dense_labels.shape[1]*args.n_ways)
            
                    dense_emb = nn.Embedding.from_pretrained(labels).cuda()
                ...
                for epsilon in epsilons:
                    ...
                    if args.dual:
                        print("dense")
                        val, _ = attack_low_dim.test_fgsm_untargeted(
                            dual.DenseWrapper(model), device, loader_attack, epsilon, mels=labels,
                            emb=dense_emb)
                        log_("test_fgsm_untargeted_dense",val,epsilon)
                        val, _ = attack_low_dim.test_fgsm_targeted(
                            dual.DenseWrapper(model), args.num_classes,device, loader_attack, epsilon, mels=labels,
                            emb=dense_emb)
                        log_("test_fgsm_targeted_dense",val,epsilon)
                        val, _ = attack_low_dim.test_iterative_untargeted(dual.DenseWrapper(model), device, 
                            loader_attack, mels=labels, epsilon=epsilon, alpha=alpha, num_steps=num_steps,emb=dense_emb)
                        log_("test_iterative_untargeted_dense",val,epsilon)                        
                        val, _ = attack_low_dim.test_iterative_targeted(dual.DenseWrapper(model), args.num_classes, device, 
                            loader_attack, mels=labels, epsilon=epsilon, alpha=alpha, num_steps=num_steps,emb=dense_emb)
                        log_("test_iterative_targeted_dense",val,epsilon)

    except KeyboardInterrupt:
        pass
    if best_metric is not None:
        _logger.info('{2}*** Best metric: {0} (epoch {1})'.format(best_metric, best_epoch,args.name))
        with open('results.txt', 'a') as file:
            file.write('{2}*** Best metric: {0} (epoch {1})\n'.format(best_metric, best_epoch,args.name))
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if dual_mode:
                output = model(input,dual_mode)
                loss = loss_fn(output, target,dual_mode)
            else:
                output = model(input)
        ...
        if not args.distributed:
            if dual_mode:
                losses_m.update(sum(loss).item(), input.size(0))
            else:
...
def validate(
    ...
    if log_dense:
        if args.n_ways != 1:
            labels = F.one_hot(loss_fn.dense_labels, args.n_ways)
            labels= labels.reshape(loss_fn.dense_labels.shape[0],loss_fn.dense_labels.shape[1]*args.n_ways)
        else:
            labels = loss_fn.dense_labels
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if log_dense:
                dense_acc1 = count_correct_nn(dense_output, dense_target, labels)/input.shape[0]
            if args.distributed:
                reduced_loss = utils.reduce_tensor(loss.data, args.world_size)
