...
def is_nn_target(output, target, mels):
    ...
    mse_dists = mse(output.repeat(num_classes, 1).cuda(), mels.cuda()).mean(-1)  # low-dim label is 1D
    output_NN = mels[torch.argmin(mse_dists)]
    return (output_NN.cuda() - target.cuda()).abs().sum() < 1e-5
...
# FGSM (untargeted)
def test_fgsm_untargeted(model, device, test_loader, epsilon, mels,emb):
    # Accuracy counter
    correct = 0
    adv_examples = []
...
# FGSM (targeted)
def test_fgsm_targeted(model, num_classes, device, test_loader, epsilon, mels,emb):
    mels=mels.to(device)
    # Accuracy counter
    correct = 0
    adv_examples = []
...
def test_iterative_untargeted(
    model, device, test_loader, mels, epsilon, alpha, num_steps,emb
):
    mels=mels.to(device)
    # Accuracy counter
    correct = 0
...
def test_iterative_targeted(
    model, num_classes, device, test_loader, mels, epsilon, alpha, num_steps,emb
):
    mels=mels.to(device)
